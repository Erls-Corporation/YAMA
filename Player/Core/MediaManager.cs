	...
	public static class MediaManager
	{
		...
		public static double Buffer
		{
			get
			{
				...
				{
					object loaded = InvokeScript("getBytesLoaded");
					object total = InvokeScript("getBytesTotal");
					//object start = InvokeScript("getBytesStart");
					if (loaded != null && total != null)
					{
						double l = Convert.ToDouble(loaded);
						double t = Convert.ToDouble(total);
						//double s = Convert.ToDouble(start);
						double b = t == 0 ? 0 : (10 * l / t);
						return b;
					}
				}
				return 0.0;
		...
		public static bool IsSupported(String path)
		{
			if (YouTubeManager.IsYouTube(path) || 
				SoundCloudManager.IsSoundCloud(path) || 
				JamendoManager.IsJamendo(path))
				return true;
		...
		public static TrackData GetTrack(string path)
		{
			switch (GetType(path))
			{
				...
				case TrackType.Jamendo:
					return JamendoManager.CreateTrack(path);
		...
		public static TrackType GetType(string path)
		{
			...
			else if (JamendoManager.IsJamendo(path))
		...
		private static void Start(TrackData track)
		{
			...
			ThreadStart PlayThread = delegate()
			{
				...
				switch (type)
				{
					...
					case TrackType.Jamendo:
						stream = Bass.BASS_StreamCreateURL(JamendoManager.GetStreamURL(track), 0, BASSFlag.BASS_SAMPLE_FLOAT, null, IntPtr.Zero);
						break;
				...
				if ((YouTubeManager.IsYouTube(track.Path) && YouTubeManager.HasFlash) ||
					stream != 0)
					SettingsManager.MediaState = MediaState.Playing;
				loadedTrack = null;
		...
		private static void Tick(object sender, EventArgs e)
		{
			try
			{
				...
				if (SettingsManager.CurrentTrack != null)
				{
					...
					if ((youtube && (int)ps != 0) || Bass.BASS_ChannelIsActive(stream) == BASSActive.BASS_ACTIVE_PLAYING)
					{
						double pos = Position;
						double len = Length;
						if (pos < 0) pos = 0;
					...
					{
						if (Bass.BASS_ChannelIsActive(stream) != BASSActive.BASS_ACTIVE_STALLED)
							SettingsManager.MediaState = MediaState.Ended;
					}
				}
			}
