...
namespace Stoffi.Core
{
	...
	public static class PlaylistManager
	{
		...
		public static bool IsSupported(String path)
		{
			...
			else if (YouTubeManager.IsPlaylist(path)) return true;
		...
		{
			return CreatePlaylist(name, 0, 0, interactive);
		}
		...
		public static PlaylistData LoadPlaylist(String filename)
		{
			if (filename.StartsWith(pathPrefix))
			{
				try
				{
					U.L(LogLevel.Debug, "SERVICE", "Retrieving playlist");
					string url = String.Format("{0}stoffiplayer.com/playlists/{1}.json",
						(SettingsManager.Channel == "stable" ? "https://www" : "http://beta"),
						filename.Substring(pathPrefix.Length));
					...
					try
					{
						...
						else
						{
							...
							if (json != null)
							{
								return ServiceManager.MergePlaylistObject(json);
							}
						}
					}
					catch (WebException exc)
					{
						...
						U.L(LogLevel.Error, "SERVICE", "Error while contacting server: " + exc.Message);
					}
				}
				catch (Exception e)
				{
