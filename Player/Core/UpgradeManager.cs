...
namespace Stoffi.Core
{
	...
	public static class UpgradeManager
	{
		...
		public static void Finish()
		{
			...
			foreach (FileInfo file in files)
			{
				if (file.Name == "Settings Migrator.dll")
				{
					...
					foreach (Type type in assembly.GetTypes())
					{
						...
						foreach (string userFolder in Directory.GetDirectories(usersFolder))
						{
							string configFilePath = Path.Combine(userFolder, "AppData", "Local", configCompanyFolder, configBaseFolder, configVersionFolder, "user.config");
							string user = new DirectoryInfo(userFolder).Name;
							if (!File.Exists(configFilePath)) continue;

							U.L(LogLevel.Debug, "UPGRADE", "Migrating settings for user " + user);


							// find user settings file
							if (!System.IO.File.Exists(configFilePath))
							{
								U.L(LogLevel.Warning, "UPGRADE", String.Format("No settings file found at {0}", configFilePath));
								continue;
							}

							object[] arguments = new object[] { configFilePath, configFilePath + ".new" };

							U.L(LogLevel.Debug, "UPGRADE", String.Format("Invoking DLL procedure"));
							type.InvokeMember("Migrate", BindingFlags.Default | BindingFlags.InvokeMethod, null, migratorObject, arguments);

							// move settings file to current folder
							U.L(LogLevel.Debug, "UPGRADE", String.Format("Move settings file to: " + configFilePath));
							if (File.Exists(configFilePath + ".old"))
								File.Delete(configFilePath + ".old");
							if (File.Exists(configFilePath))
								File.Move(configFilePath, configFilePath + ".old");
							File.Move(configFilePath + ".new", configFilePath);
						}
				...
				else
				{
					...
					if (File.Exists(newFile))
						File.Move(newFile, bakFile);

					U.L(LogLevel.Debug, "UPGRADE", String.Format("Backing up {0} to {1}", newFile, bakFile));
			...
			{
				string configFilePath = Path.Combine(userFolder, "AppData", "Local", configCompanyFolder, configBaseFolder, configVersionFolder, "user.config");
				string newConfigFolder = Path.Combine(userFolder, "AppData", "Local", configCompanyFolder, configBaseFolder, newVersion);
				if (File.Exists(configFilePath))
				{
					...
					File.Copy(configFilePath, newSettingsFile);
				}
			}
