class DevicesController < ApplicationController
	...
	def show
		...
		@title = "Device '#{h(@device.name)}'"
		@description = "The device named #{h(@device.name)} which is connected to your account"
		respond_with @device
	end
	...
	def edit
		@device = current_user.devices.find(params[:id])
		@title = "Edit device '#{h(@device.name)}'"
		@description = "Modify the device named #{h(@device.name)} which is connected to your account"
	end
	...
	def create
		...
		if @device.save
			...
			SyncController.send('create', @device)
		end
		respond_with(@device)
	end
	...
	def update
		...
		SyncController.send('update', @device) if success
		respond_with(@device)
	end
	...
	def destroy
		@device = current_user.devices.find(params[:id])
		SyncController.send('delete', @device)
		@device.destroy
		respond_with(@device)
	end
