class SharesController < ApplicationController
	...
	def create
		...
		@share.object = params[:object] if params[:object]
		...
		#success = @share.save
		#SyncController.send('create', @share) if success
		respond_with @share
	end
	...
	def update
		...
		SyncController.send('update', @share) if success
		respond_with @share
	end
	...
	def destroy
		render :status => :forbidden and return if ["xml","json"].include?(params[:format])
		@share = current_user.shares.find(params[:id])
		SyncController.send('delete', @share)
		@share.destroy
		respond_with @share
	end
end
