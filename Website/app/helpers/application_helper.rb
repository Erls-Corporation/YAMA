module ApplicationHelper
	...
	def button_tag(text, destination, image = "", options = {})
		...
		content = content_tag(:h1, text, :class => h1class) + sub
		...
		if options[:type] == :link
			...
			link_to_function(content, destination, :class => c, :style => s)
		elsif options[:type] == :form
			destination = "" unless destination
			link_to_function(content, "#{destination}$(this).closest('form').submit()", :class => c, :style => s)
		end
	end
	...
	def item(text, resource, field, options = {})
		...
		text = content_tag(:p, text, :data => { :field => field })
		...
		content_tag(:li, content, :data => { :object => id }, :class => options[:class])
	end
	...
	def editable_label(resource, property, options = {})
		...
		label = content_tag(options[:tag], h(val),
